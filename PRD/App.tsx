...
import { HistoryEntry, Profile, Dose } from './types';
// `Medication` is implemented as a class in `hooks/useMedicationStore.ts`.
// Use a permissive alias here to avoid an import that doesn't exist in `types.ts`.
type Medication = any;
...
interface RegisterScreenProps {
  onRegister: (name: string, relation: string, email: string, password: string) => void;
  onBackToLogin: () => void;
}
...
const App: React.FC = () => {
  ...
  const handleLogin = () => {
    ...
    try {
      ...
      console.error('Falha ao ler usuário salvo', err);
      alert('Erro ao tentar autenticar.');
    }
  };

  // Cadastro de usuário: cria perfil único e salva credenciais locais (simulação)
  const handleRegister = (name: string, relation: string, email: string, password: string) => {
    // Salva usuário (simulado)
    try {
      localStorage.setItem('medalert-user', JSON.stringify({ name, relation, email, password }));
    } catch (err) {
      console.warn('Não foi possível salvar usuário no localStorage', err);
    }
    // Adiciona perfil ao store (somente nome e relação)
